...
struct expty transVar(Tr_level level, S_table venv, S_table tenv, A_var v) {
	switch (v->kind) {
		...
		case A_fieldVar: {
			struct expty exp = transVar(level, venv, tenv, v->u.field.var); 
			Ty_ty t = exp.ty;
			if (t->kind != Ty_record) {
	...
	switch (a->kind) {
		...
		case A_seqExp: {
			...
			for (; el->tail; el = el->tail)
				transExp(level, venv, tenv, el->head);
			return transExp(level, venv, tenv, el->head);
		}
		case A_assignExp: {
		...
		case A_letExp: {
			...
			for (lt = a->u.let.decs; lt; lt = lt->tail)
				transDec(level, venv, tenv, lt->head);
	...
	switch (d->kind) {
		...
		case A_functionDec: {
			...
			for (fl = d->u.function; fl; fl = fl->tail) {
				...
				struct expty exp = transExp(ent->u.fun.level, venv, tenv, fun->body);
